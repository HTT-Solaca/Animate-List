...
class Selector(tk.Toplevel):
    def __init__(self, parent, list, update_callback):
        ...
        self.selected_tags_display = tk.Label(self, text="")
        ...
        self.tag_checkboxes = []
        self.original_pack_options = {}  # 记录原始包装选项

        for tag in list:
            checkbox = ttk.Checkbutton(self.tag_inner_frame, text=tag, command=lambda t=tag: self.toggle_tag(t))
            checkbox.state(['!alternate'])
            self.tag_checkboxes.append(checkbox)
            self.original_pack_options[checkbox] = {"side": "top"}  # 记录原始包装选项
...
class Gui(Tooltip,ASSIST,Selector):
    ...
    def set(self):
        ...
        self.btshowall=tk.Button(self.pwset,height =30,width = 30,image=self.showallpng,
            bg='white',relief=GROOVE,activebackground='orange',command= lambda:print(self.selected_T))
    ...
    def create_sheetbtn(self):
        ...
        for sheet_name in self.sheet_names:
            ...
            button.bind("<Button-1>", lambda event, b=button: self.sheetbtn_color(event, b))
            button.bind("<Button-3>", lambda event, b=button: self.sheetbtn_menu(event, b))
            button.grid(row = i)
            i+=1
    '''表点击时颜色更改'''
    def sheetbtn_color(self, event, button):
        for child in self.pwsheet.winfo_children():
            if isinstance(child, tk.Button):
                child.config(bg='white')
        button.config(bg='#70AD47')
    '''表右键'''
    ...
    def settabletagf(self):
        Selector(self.root,['empty'],self.update_selected)
    def update_selected(self,tags):
